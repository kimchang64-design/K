...
with main_tab2:
    ...
    THEME_DATA = {
        "광케이블/광섬유": {
            ...
            "up_down": "상승 13 / 하락 0",
            "stocks": [
                ("대한광통신", "010170", 1850, 14.50, 12500000, 231, 2100, 1620, 1780),
                ("광전자", "017900", 2450, 9.80, 4200000, 102, 1400, 2100, 2350),
                ("RF머트리얼즈", "327260", 33750, 29.80, 3480000, 1174, 3920, 16572, 28500),
            ],
        },
        "전선": {
            "change": "+8.91%",
            "up_down": "상승 8 / 하락 0",
            "stocks": [
                ("대한전선", "001440", 14200, 8.50, 18200000, 2584, 18500, 12800, 13900),
                ("KBI메탈", "024840", 2150, 12.10, 8900000, 191, 850, 1850, 2020),
                ("LS마린솔루션", "028670", 19800, 6.20, 3100000, 613, 5100, 17200, 19100),
            ],
        },
        "통신장비": {
            "change": "+7.51%",
            "up_down": "상승 44 / 하락 0",
            "stocks": [
                ("이노인스트루먼트", "215790", 988, 30.00, 8876213, 85, 1923, 760, 910),
                ("CS", "065770", 1990, 29.98, 723915, 13, 975, 1530, 1820),
                ("오이솔루션", "058610", 25150, 29.84, 115747, 29, 2336, 19800, 23400),
                ("에이스테크", "088800", 2260, 25.56, 230131, 4, 1518, 1810, 2100),
                ("케이엠더블유", "032500", 27050, 21.57, 9348459, 243, 7686, 22100, 25800),
                ("서진시스템", "178320", 35000, 16.86, 302016, 109, 9022, 29800, 33500),
            ],
        },
        "반도체 대표주(생산)": {
            "change": "+6.94%",
            "up_down": "상승 3 / 하락 0",
            "stocks": [
                ("삼성전자", "005930", 72500, 4.50, 28500000, 20600, 4320000, 68500, 71200),
                ("SK하이닉스", "000660", 188500, 8.90, 8900000, 16700, 1370000, 165000, 181000),
                ("DB하이텍", "000990", 48500, 7.20, 1250000, 606, 21500, 43200, 46800),
            ],
        },
        "5G(5세대 이동통신)": {
            "change": "+6.59%",
            "up_down": "상승 50 / 하락 2",
            "stocks": [
                ("RFHIC", "218410", 18900, 12.50, 2100000, 396, 4800, 16200, 17900),
                ("쏠리드", "050890", 6100, 7.80, 1850000, 112, 3700, 5400, 5900),
                ("에프알텍", "083450", 3120, 15.20, 3100000, 96, 820, 2600, 2980),
            ],
        },
        "온디바이스 AI": {
            "change": "+6.57%",
            "up_down": "상승 18 / 하락 1",
            "stocks": [
                ("제주반도체", "080220", 24500, 18.20, 14500000, 3550, 8400, 19500, 22800),
                ("리노공업", "058470", 210000, 5.40, 620000, 1302, 31900, 192000, 205000),
                ("칩스앤미디어", "094360", 28900, 11.20, 2800000, 809, 5800, 24100, 27300),
            ],
        },
    }

    # 1. 인기 업종·테마 TOP 1위 ~ 5위 카드리스트
        ...
        if selected_theme:
            ...
            for idx, item in enumerate(stocks_list, start=1):
                ...
                <tr style="border-bottom: 1px solid #f0f0f0; height: 45px; font-size: 13px;">
                    <td style="text-align: center; color: #666; width: 40px;">{idx}</td>
                    <td style="font-weight: bold; color: #111; padding-left: 8px; width: 110px;">{stock_name_val}</td>
                    <td style="text-align: center; width: 90px;">
                        <button onclick="navigator.clipboard.writeText('{stock_code_val}');" 
                                style="padding: 3px 6px; background-color: #f1f3f5; color: #333; border: 1px solid #ced4da; border-radius: 4px; cursor: pointer; font-family: monospace; font-size: 11px; font-weight: bold;">
                            📋 {stock_code_val}
                        </button>
                    </td>
                    <td style="text-align: right; padding-right: 8px; font-weight: bold; width: 90px;">{price_val}</td>
                    <td style="text-align: center; width: 115px; background-color: #fff9db;">
                        <button onclick="navigator.clipboard.writeText('{day_vwap_val}');" 
                                style="padding: 3px 6px; background-color: #ffe066; color: #000; border: 1px solid #fcc419; border-radius: 4px; cursor: pointer; font-family: monospace; font-size: 11px; font-weight: bold;">
                            📋 {day_vwap_val:,}
                        </button>
                    </td>
                    <td style="text-align: center; width: 115px; background-color: #e7f5ff;">
                        <button onclick="navigator.clipboard.writeText('{min3_vwap_val}');" 
                                style="padding: 3px 6px; background-color: #a5d8ff; color: #000; border: 1px solid #74c0fc; border-radius: 4px; cursor: pointer; font-family: monospace; font-size: 11px; font-weight: bold;">
                            📋 {min3_vwap_val:,}
                        </button>
                    </td>
                    <td style="text-align: right; padding-right: 8px; color: #d32f2f; font-weight: bold; width: 75px;">{change_val}</td>
                </tr>
                """
            ...
            <div style="overflow-x: auto; border: 1px solid #e0e0e0; border-radius: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
                <table style="width: 100%; border-collapse: collapse; background-color: #ffffff;">
                    <thead>
                        <tr style="background-color: #fafafa; border-bottom: 2px solid #e0e0e0; color: #555; font-size: 12px; height: 40px;">
                            <th style="text-align: center; width: 40px;">순위</th>
                            <th style="text-align: left; padding-left: 8px; width: 110px;">종목명</th>
                            <th style="text-align: center; width: 90px;">종목코드</th>
                            <th style="text-align: right; padding-right: 8px; width: 90px;">현재가</th>
                            <th style="text-align: center; width: 115px; background-color: #fff3bf; color: #d9480f;">일봉 세력평단</th>
                            <th style="text-align: center; width: 115px; background-color: #d0ebff; color: #1864ab;">3분봉 세력평단</th>
                            <th style="text-align: right; padding-right: 8px; width: 75px;">전일대비</th>
                        </tr>
                    </thead>
                    <tbody>
                        {table_rows_html}
